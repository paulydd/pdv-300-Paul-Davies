...
export function checkCollision(A: Entity, B: Entity): Collision | null {
    ...
    return { entityA: A, entityB: B, ...result };
}
...
function checkPolyCollision(pA: Vec2[], pB: Vec2[]): { normal: Vec2; loc: Vec2; penetration: number; } | null {
    ...
    const axes: Vec2[] = [];
    const candidates: { axis: Vec2; overlap: number; }[] = [];
    ...
    if (dirLenSq > 1e-8) {
        ...
        let bestDot = Math.abs((bestAxis.x * dirX + bestAxis.y * dirY) * invDirLen);
        const ALIGN_THRESHOLD = 0.35;
        const MAX_OVERLAP_RATIO = 4;
        if (bestDot < ALIGN_THRESHOLD) {
            let chosenAxis = bestAxis;
            let chosenOverlap = minOverlap;
            let chosenDot = bestDot;
            let found = false;
            for (let i = 0; i < candidates.length; i++) {
                const candidate = candidates[i];
                const rawDot = (candidate.axis.x * dirX + candidate.axis.y * dirY) * invDirLen;
                const alignment = Math.abs(rawDot);
                if (alignment < ALIGN_THRESHOLD) continue;
                if (candidate.overlap > minOverlap * MAX_OVERLAP_RATIO) continue;
                if (
                    !found ||
                    candidate.overlap < chosenOverlap - 1e-6 ||
                    (Math.abs(candidate.overlap - chosenOverlap) <= 1e-6 && alignment > chosenDot)
                ) {
                    chosenAxis = candidate.axis;
                    chosenOverlap = candidate.overlap;
                    chosenDot = alignment;
                    found = true;
                }
            }
...
export function resolveCollision(collision: Collision) {
    const { entityA: A, entityB: B, normal, loc, penetration } = collision;
    const bodyA = A.body;
    const bodyB = B.body;
    ...
    const applyPositionalCorrection = () => {
        if (depth <= 0) return;
        const correctionMag = percent * depth / invMassSum;
